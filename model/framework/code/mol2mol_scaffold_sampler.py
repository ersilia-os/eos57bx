...
class Mol2MolScaffoldSampler:
    ROOT = os.path.dirname(os.path.abspath(__file__))
    CHECKPOINT = os.path.join(ROOT, "..", "..", "checkpoints")
    SCAFFOLD_MODEL = os.path.realpath(
        os.path.join(CHECKPOINT, "mol2mol_scaffold.prior")
    )
    SCAFFOLD_GENERIC_MODEL = os.path.realpath(
        os.path.join(CHECKPOINT, "mol2mol_scaffold_generic.prior")
    )
    ...
    def __init__(self, batch_size):
        ...
        # Creating adapter
        scaffold_agent, _, _ = create_adapter(
            self.SCAFFOLD_MODEL, "inference", torch.device("cpu")
        )
        scaffold_generic_agent, _, _ = create_adapter(
            self.SCAFFOLD_GENERIC_MODEL, "inference", torch.device("cpu")
        )

        # Creating sampler.
        self.scaffold_sampler = samplers.Mol2MolSampler(
            scaffold_agent,
        ...
        self.scaffold_generic_sampler = samplers.Mol2MolSampler(
            scaffold_generic_agent,
            batch_size=self.batch_size,
            sample_strategy=sample_strategy,
            isomeric=isomeric,
            randomize_smiles=randomize_smiles,
    ...
    def generate(
        ...
        for smi in input_smiles:
            try:
                with torch.no_grad():
                    sampled = sampler.sample([smi])

                _, valid_idxs = self.chemistry.conversions.smiles_to_mols_and_indices(
                    sampled.items2
                )
                valid_scores = sampler.calculate_tanimoto([smi], sampled.items2)
